...
def analyze_frequency_components_torch(feature_map):
    ...
    return feature_map_freq, torch.abs(feature_map_freq)



def identify_high_low_masks(mag_space):
    ...
    def generate_mask(threshold, C, T, H, W):
        ...
        mask[mask>0] = 1
        return mask.unsqueeze(0).repeat(C, 1, 1, 1)
    ...
    high_mask = generate_mask(high_thresh, C, T, H, W)
    mid_high_mask = generate_mask(mid_high_thresh, C, T, H, W) - high_mask
    mid_low_mask = generate_mask(mid_low_thresh, C, T, H, W) - generate_mask(mid_high_thresh, C, T, H, W)
    low_mask = generate_mask(low_thresh, C, T, H, W) - generate_mask(mid_low_thresh, C, T, H, W)
    ...
    high_freq_power = extract_mean_power_mask(mag_space, high_mask)
    mid_high_power = extract_mean_power_mask(mag_space, mid_high_mask)
    mid_low_power = extract_mean_power_mask(mag_space, mid_low_mask)
    low_freq_power = extract_mean_power_mask(mag_space, low_mask)
...
def batch_ddim_sampling(model, cond, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1.0,\
    ...
    for _ in range(n_samples):
        ...
        init_noise = model.decode_first_stage_2DAE(init_noise)
        freq, real_freq = analyze_frequency_components_torch(batch_images.detach().cpu().float())
        high_freq_content, mid_high_freq_content, mid_low_freq_content, low_freq_content = identify_high_low_masks(real_freq)

        freq_noise, real_freq_noise = analyze_frequency_components_torch(init_noise.detach().cpu().float())
        high_freq_content_noise, mid_high_freq_content_noise, mid_low_freq_content_noise, low_freq_content_noise = identify_high_low_masks(real_freq_noise)
...
def save_videos(batch_tensors, savedir, filenames, fps=10, frames=16):
    # b,samples,c,t,h,w
    ...
    for idx, vid_tensor in enumerate(batch_tensors):
        ...
        savepath = os.path.join(savedir, f"{filenames[idx]}_{frames}_global.mp4")
