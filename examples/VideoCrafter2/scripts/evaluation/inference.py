...
def run_inference(args, gpu_num, gpu_no, **kwargs):
    ...
    for frame_index in range(16, args.frames, 4):
        list_index = list(range(frame_index-16, frame_index+4-16))
    ...
    for idx in range(0, n_rounds):
        ...
        ## b,samples,c,t,h,w
        filenames = [prompt.replace(' ', '_') + f"{frames}" for prompt in prompts]
        save_videos(batch_samples, args.savedir, filenames, fps=args.savefps, frames=frames)
