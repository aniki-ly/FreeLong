...
class CrossAttention(nn.Module):
    ...
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0., 
        ...
        self.local_mask = self.get_local_mask(128, 16)
    ...
    def temoral_forward(self, x, context=None, mask=None):
        h = self.heads
        ...
        def create_adjusted_attention_mask(seq_length, window_size):
            mask = torch.zeros((seq_length, seq_length), dtype=torch.bool)
        ...
        def apply_attention_mask(attention_scores, mask):
            return attention_scores.masked_fill(mask == False, float('-inf'))
            # attention_scores = attention_scores * mask + 0.1 * attention_scores * (~mask)
            # return attention_scores

        seq_length = sim.size(-1)
        mask = self.local_mask.to(sim.device)
        expanded_mask = mask.unsqueeze(0).expand(sim.size(0), -1, -1)
        sim = apply_attention_mask(sim, expanded_mask)
...
class TemporalBasicTransformerBlock(nn.Module):
    ...
    def global_temp_attn(self, hidden_states, norm_temp, attn_temp, video_length=16):

        # for global attention
        norm_hidden_states = norm_temp(hidden_states)
        hidden_states = attn_temp(norm_hidden_states) + hidden_states
    ...
    def local_attn(self, hidden_states, norm_temp, attn_temp, video_length=16, reverse=False):

        # for global attention
        norm_hidden_states = norm_temp(hidden_states)
        hidden_states = attn_temp.temoral_forward(norm_hidden_states) + hidden_states
    ...
    def _forward(self, x, context=None, mask=None):
        ...
        local_feat = self.local_attn(x, self.norm1, self.attn1, video_length=video_length)
        global_feat = self.global_temp_attn(x, self.norm1, self.attn1, video_length=video_length)
        x = self.combine(global_feat, local_feat, video_length)


        local_feat = self.local_attn(x, self.norm2, self.attn2, video_length=video_length)
        global_feat = self.global_temp_attn(x, self.norm2, self.attn2, video_length=video_length)
        x = self.combine(global_feat, local_feat, video_length)
